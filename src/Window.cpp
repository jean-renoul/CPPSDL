...
Window::Window() {
    ...
    if (!backgroundMusic.openFromFile("../assets/byte-blast-8-bit-arcade-music-background-music-for-video-33-second-208777.mp3")) {  // Utilise un fichier .ogg ou .wav
        std::cerr << "Error loading music" << std::endl;
    } else {
        backgroundMusic.setLoop(true);  // La musique se répète en boucle
        backgroundMusic.play();         // Commence à jouer la musique
    }
...
void Window::renderGameOver(int currentScore) {
    // Arrêter la musique si le jeu est terminé
