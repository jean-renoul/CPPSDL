...
Grid::Grid() : highScore(0) {
    ...
    Score scoreManager;
    highScore = scoreManager.readScore("highscore.txt");
}
...
void Grid::setBlock(int row, int col, const Bloc& bloc) {
    blocks[row][col] = bloc;
    highScore = std::max(highScore, bloc.getValue());
}

int Grid::getHighScore() const {
    return highScore;
}

void Grid::render(SDL_Renderer* renderer, TTF_Font* font, Score& score) {
    
    SDL_Color textColor = { 0, 0, 0, 255 }; 
    std::string scoreText = "Your best score: " + std::to_string(highScore);
    ...
    SDL_Rect textRect = { 3, 3, textWidth, textHeight }; 
...
void Grid::drawBlock(SDL_Renderer* renderer, int x, int y, int value, TTF_Font* font) {
    SDL_Rect cellRect = { x * CELL_SIZE, y * CELL_SIZE + 30, CELL_SIZE, CELL_SIZE }; 
...
SDL_Color Grid::getBlockColor(int value) const {
    switch (value) {
        ...
        default: return { 0xDD, 0xDD, 0xDD, 255 }; // Defalt color
    }
}
